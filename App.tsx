...
const App: React.FC = () => {
  ...
  const handleFinalizeSend = () => {
    ...
    if (card.imageUrl) {
      // For base64 or blob URLs, use the download helper
      const link = document.createElement('a');
      link.href = card.imageUrl;
      link.download = `${card.name || 'pup'}_image.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    }
    ...
    // Prepare email
    const subject = encodeURIComponent(`Bone Battle Card: ${card.name}`);
    
    let bodyText = `Hi Joker,\n\nHere's my card request for the Bone Battle game.\n\n`;
    bodyText += `I'm ${card.name}, my favorite color is ${card.hoodColor}, and I'm from ${card.country}.\n\n`;
    bodyText += `You can contact me on my social media: ${card.socialLink}\n\n`;
    
    bodyText += `IMPORTANT: I have attached the downloaded .txt file (ps_data) and the image file to this email.\n\n`;
    
    if (newsletterSubscribe) {
        bodyText += `[YES] I would like to subscribe to the newsletter for Bone Battle updates.\n\n`;
    }
    ...
    // Open Mail
    window.location.href = `mailto:albert@codehunterlab.com?subject=${subject}&body=${body}`;
  };
            ...
            <div className="flex flex-col items-center md:items-end text-center md:text-right">
                ...
                <div className="flex flex-col gap-2 items-center md:items-end">
                    ...
                    <div className="flex items-center gap-2 text-slate-600">
                         <Code size={14} />
                         <span className="text-xs font-mono font-semibold tracking-wider text-slate-500">CodeHunter Lab</span>
                    </div>
                </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            >
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-end gap-3 sticky bottom-0 rounded-b-2xl">
                    ...
                    <button
                        type="submit"
                        className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg shadow-blue-900/20 transition-all transform hover:scale-105"
                    >
                        <Mail size={16} />
                        Continue to Email
                    </button>
