...
// ==========================================
// JSON IMPORT (optional pre-fill)
// ==========================================
var jsonWin = new Window("dialog", "Import Card Data (JSON)");
jsonWin.orientation = "column";
jsonWin.alignChildren = "fill";
jsonWin.add("statictext", undefined, "Paste JSON below, or click Skip to use defaults:");
var jsonInput = jsonWin.add("edittext", undefined, "", {multiline: true});
jsonInput.preferredSize = [420, 200];
var jsonBtnGrp = jsonWin.add("group");
jsonBtnGrp.alignment = "right";
var jsonLoadBtn = jsonBtnGrp.add("button", undefined, "Load JSON");
var jsonSkipBtn = jsonBtnGrp.add("button", undefined, "Skip", {name: "cancel"});
...
jsonLoadBtn.onClick = function() {
    try {
        ...
        if (d.color         !== undefined) pupColor   = String(d.color);
        if (d.year          !== undefined) year       = String(d.year);
        if (d.month         !== undefined) month      = String(d.month);

        // pawsday: manual format uses pawsDayYear/pawsDayMonth,
        //          app export uses pawsday "YYYY.MM"
        if (d.pawsDayYear  !== undefined) { pawsDayYear  = String(d.pawsDayYear); }
        else if (d.pawsday !== undefined) { var _pd = String(d.pawsday).split('.'); if (_pd[0]) pawsDayYear  = _pd[0]; }
        if (d.pawsDayMonth !== undefined) { pawsDayMonth = String(d.pawsDayMonth); }
        else if (d.pawsday !== undefined) { var _pd2 = String(d.pawsday).split('.'); if (_pd2[1]) pawsDayMonth = _pd2[1]; }

        if (d.height !== undefined) initialHeight = String(d.height);

        // shoe: manual format uses partial EU number ("44"),
        //       app export uses full string ("44EU / 11US") → extract leading number
        if (d.shoe           !== undefined) { initialShoe = String(d.shoe); }
        else if (d.shoe_size !== undefined) { var _sm = String(d.shoe_size).match(/^(\d+(?:\.\d+)?)/); if (_sm) initialShoe = _sm[1]; }

        // flag: manual format uses flag name ("germany"),
        //       app export uses ISO 2-letter code ("DE") → map to flag name
        if (d.flag !== undefined) { initialFlag = String(d.flag); }
        else if (d.country !== undefined) {
            var _cmap = {
                'PT':'portugal','GB':'UK','RU':'russia','AR':'argentinia','BG':'bulgarien',
                'CZ':'czech','RO':'romania','ZA':'south africa','MX':'mexiko','EC':'ecuador',
                'CH':'schweiz','FI':'finland','NO':'norway','IE':'irland','NZ':'NZ',
                'US':'USA','SI':'slovenia','CA':'canada','SK':'slovakia','SE':'sweden',
                'AU':'australia','CO':'colombia','BE':'belgium','ES':'spain','PL':'poland',
                'IL':'israel','NL':'netherlands','DE':'germany','IT':'italy','HU':'hungary',
                'FR':'france','AT':'austria','WA':'wales'
            };
            var _cf = _cmap[String(d.country).toUpperCase()];
        ...
        if (g.leather    !== undefined) pLeather = String(g.leather);
        // app export uses "skate" for sneaker
        if (g.sneaker    !== undefined) { pSneaker = String(g.sneaker); } else if (g.skate     !== undefined) { pSneaker = String(g.skate); }
        // app export uses "underwear" for jocks
        if (g.jocks      !== undefined) { pJocks   = String(g.jocks);   } else if (g.underwear !== undefined) { pJocks   = String(g.underwear); }
        // app export uses "suit" for furry
        if (g.furry      !== undefined) { pFurry   = String(g.furry);   } else if (g.suit      !== undefined) { pFurry   = String(g.suit); }
        if (g.mx         !== undefined) pMx      = String(g.mx);
        // app export uses "sports" for sportswear
        if (g.sportswear !== undefined) { pSportswear = String(g.sportswear); } else if (g.sports !== undefined) { pSportswear = String(g.sports); }
        if (g.tactical   !== undefined) pTactical = String(g.tactical);

        var k = d.kinks || {};
        if (k.outdoor    !== undefined) pOutdoor  = String(k.outdoor);
        if (k.sniffing   !== undefined) pSniffing = String(k.sniffing);
        if (k.edging     !== undefined) pEdging   = String(k.edging);
        if (k.fisting    !== undefined) pFisting  = String(k.fisting);
        if (k.abdl       !== undefined) pAbdl     = String(k.abdl);
        if (k.toys       !== undefined) pToys     = String(k.toys);
        if (k.cuckolding !== undefined) pCuck     = String(k.cuckolding);
        if (k.powerplay  !== undefined) pPower    = String(k.powerplay);
        if (k.chastity   !== undefined) pChastity = String(k.chastity);
        if (k.bdsm       !== undefined) pBdsm     = String(k.bdsm);
        if (k.verbal     !== undefined) pVerbal   = String(k.verbal);
        if (k.dirty      !== undefined) pDirty    = String(k.dirty);
...
jsonWin.show();
// if skipped/closed, all vars stay at their defaults
