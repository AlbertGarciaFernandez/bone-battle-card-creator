...
const CardPreview: React.FC<CardPreviewProps> = ({ data }) => {
  ...
  // Helper to render bones
  const renderBones = (count: number = 0) => {
    return (
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((v) => (
          <Bone
            key={v}
            size={12}
            className={`transform -rotate-45 ${v <= count ? 'text-white fill-white' : 'opacity-0'}`} 
          />
        ))}
      </div>
    );
  ...
  const totalBones = React.useMemo(() => {
    let sum = 0;
    Object.values(gear).forEach((v) => sum += (v as number));
    Object.values(kinks).forEach((v) => sum += (v as number));
    return sum;
  }, [gear, kinks]);
  ...
  return (
    <div className={`relative w-[360px] h-auto min-h-[640px] rounded-2xl overflow-hidden bg-black flex flex-col shadow-2xl border-4 ${borderColor} font-sans`}>
            ...
            <div className="flex items-center mb-1">
                ...
                <div className="flex-1 flex gap-2 ml-1">
                    ...
                    <div className="flex-1 space-y-0.5 border-r border-white/10 pr-2">
                        {GEAR_LEFT.map(cat => (
                            <div key={cat} className="flex items-center justify-between h-4">
                                <span className="text-[10px] text-neutral-300 font-bold truncate pr-1">{cat}</span>
                                {renderBones(gear[cat] || 0)}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 space-y-0.5 pl-2">
                         {GEAR_RIGHT.map(cat => (
                            <div key={cat} className="flex items-center justify-between h-4">
                                {renderBones(gear[cat] || 0)}
                                <span className="text-[10px] text-neutral-300 font-bold truncate pl-1">{cat}</span>
                            </div>
                        ))}
                    </div>
                </div>
